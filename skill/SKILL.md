---
name: any2video
description: Deep-analyze any input (URL, GitHub repo, article, image, raw text) and produce a chi-tiết video with non-templated visuals + voice narration. Unlike a fixed-template video generator (fixed layouts + themes + a small fixed context), Claude itself does the deep extraction, writes the plan, designs per-scene HTML/CSS, then composes the mp4. Use when the user says "làm video từ <url>", "video giới thiệu repo <github>", "dùng any2video", "/any2video", or wants a deeper, less template-y video than a quick templated generator produces.
---
...
scenes:
  - id: 1
    beat: hook                      # FREE-FORM label, not a fixed enum.
                                    # Suggested beats for a repo tour: hook / framing /
                                    # demo / capability / honest-caveat / use-case-landing.
                                    # Pick whatever serves THIS specific repo's story.
    duration_sec: 5
    narration: "<flowing VN sentence(s), see principles 1-9 above>"
    visual_brief: |
      <Claude designs the layout. Must include specific concrete imagery — not "abstract shapes".
       If the narration names Cursor + Codex + Claude, the visual should show all three logos.>
    caption_html: "<short caption with <span class='kw'>keywords</span> highlighted>"
    grounded_in:                    # NEW — provenance per scene
      - "<exact quote or fact from analysis.md Evidence that this scene is built on>"
    data_props:
      <key: value>                  # values that MUST appear verbatim in narration AND in analysis.md Evidence
```

Constraints (HARD):
- 7-10 atomic scenes total (per 2.2.6 atomicity rule — split crammed scenes, target 6-10s each)
...
- `visual_brief` is unique per scene
- `data_props` values appear in both `narration` AND `analysis.md > ## Evidence`
...
For every other scene, Claude writes inline `scenes/<id>.html` using:
- **Design tokens** from `templates/design-tokens.md` (palette, font stack, grid, safe-zone, brand bars, caption overlay)
- **Scene's `visual_brief`** (Claude's own design intent)
- **Scene's `data_props`** (the real data)
- **Real `duration_sec`** (from TTS measurement)
- **Brand block** from `plan.md > meta.brand`: avatar URL, repo / channel name, tagline → render in `.brand-header` (top band) on EVERY scene for visual continuity
- **Footer block** from `plan.md > meta.footer`: list of stats (stars, forks, version, date, license) → render in `.brand-footer` (bottom band) on EVERY scene

HTML is self-contained: inline CSS, inline SVG, optional inline `<script>`. Viewport fixed 1080×1920 (9:16). Respect the 3-tier safe zones defined in `templates/design-tokens.md`.
